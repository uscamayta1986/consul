# -*- encoding: utf-8 -*-
# stub: devise-async 1.0.0 ruby lib

Gem::Specification.new do |s|
  s.name = "devise-async".freeze
  s.version = "1.0.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Sebastian Oelke".freeze, "Marcelo Silveira".freeze]
  s.date = "2017-09-20"
  s.description = "Devise Async provides an easy way to configure Devise to send its emails asynchronously using ActiveJob.".freeze
  s.email = ["dev@soelke.de".freeze, "marcelo@mhfs.com.br".freeze]
  s.homepage = "https://github.com/mhfs/devise-async/".freeze
  s.licenses = ["MIT".freeze]
  s.rubygems_version = "2.7.6.2".freeze
  s.summary = "Deliver Devise's emails in the background using ActiveJob.".freeze

  s.installed_by_version = "2.7.6.2" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<devise>.freeze, [">= 4.0"])
      s.add_runtime_dependency(%q<activejob>.freeze, [">= 5.0"])
      s.add_development_dependency(%q<activerecord>.freeze, [">= 5.0"])
      s.add_development_dependency(%q<actionpack>.freeze, [">= 5.0"])
      s.add_development_dependency(%q<actionmailer>.freeze, [">= 5.0"])
      s.add_development_dependency(%q<rspec>.freeze, ["~> 3.6"])
      s.add_development_dependency(%q<rspec-rails>.freeze, ["~> 3.6"])
      s.add_development_dependency(%q<sqlite3>.freeze, ["~> 1.3"])
      s.add_development_dependency(%q<pry>.freeze, [">= 0"])
    else
      s.add_dependency(%q<devise>.freeze, [">= 4.0"])
      s.add_dependency(%q<activejob>.freeze, [">= 5.0"])
      s.add_dependency(%q<activerecord>.freeze, [">= 5.0"])
      s.add_dependency(%q<actionpack>.freeze, [">= 5.0"])
      s.add_dependency(%q<actionmailer>.freeze, [">= 5.0"])
      s.add_dependency(%q<rspec>.freeze, ["~> 3.6"])
      s.add_dependency(%q<rspec-rails>.freeze, ["~> 3.6"])
      s.add_dependency(%q<sqlite3>.freeze, ["~> 1.3"])
      s.add_dependency(%q<pry>.freeze, [">= 0"])
    end
  else
    s.add_dependency(%q<devise>.freeze, [">= 4.0"])
    s.add_dependency(%q<activejob>.freeze, [">= 5.0"])
    s.add_dependency(%q<activerecord>.freeze, [">= 5.0"])
    s.add_dependency(%q<actionpack>.freeze, [">= 5.0"])
    s.add_dependency(%q<actionmailer>.freeze, [">= 5.0"])
    s.add_dependency(%q<rspec>.freeze, ["~> 3.6"])
    s.add_dependency(%q<rspec-rails>.freeze, ["~> 3.6"])
    s.add_dependency(%q<sqlite3>.freeze, ["~> 1.3"])
    s.add_dependency(%q<pry>.freeze, [">= 0"])
  end
end
